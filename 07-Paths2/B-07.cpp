...
bool union_(int node1, int node2, vector<int>& parents, vector<int>& ranks) {
...
int main() {
    ...
    for (int i = 1; i <= n; i++) {
        for (int j = 1; j <= m; j++) {
            ...
            vector<pair<int, int>> neighbors = {{i - 1, j - 1}, {i - 1, j}, {i, j - 1}, {i, j + 1}, {i + 1, j}, {i + 1, j + 1}};
    ...
    for (auto e : edges) {
        int u, v, c;
        tie(u, v, c) = e;
        if (union_(v, u, parents, ranks)) {
            total_honey += c;
        }
    }

    // Output the total amount of honey
    cout << total_honey << endl;
